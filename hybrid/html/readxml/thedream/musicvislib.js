var musicvislib = (() => {
	...
	var __toCommonJS = /* @__PURE__ */ ((cache4) => {
		return (module2, temp) => {
			return cache4 && cache4.get(module2) || (temp = __reExport(__markAsModule({}), module2, 1), cache4 && cache4.set(module2, temp), temp);
		};
	...
	// src/types/Note.js
	var Note = class {
		//zxx 新定义一个link参数,用作连音符标识 1连音符 0非连音符
		//zxx 新定义一个hand参数,用作左右手标识 1左手 2右手
		constructor(pitch = 0, start2 = 0, velocity = 127, channel = 0, end = null, link = 0,hand = 1) {
	...
	function get(type3, name2) {
		...
		if ((c2 = type3[i]).name === name2) {
			return c2.value;
		}
		}
	}
	function set2(type3, name2, callback) {
		for (var i = 0, n = type3.length; i < n; ++i) {
		if (type3[i].name === name2) {
	...
	var ENDING = 5;
	var ENDED = 6;
	function schedule_default(node, name2, id2, index16, group2, timing) {
	...
	function onFunction(id2, name2, listener) {
		var on0, on1, sit = start(name2) ? init : set3;
		return function () {
	...
	function styleMaybeRemove(id2, name2) {
		...
		return function () {
		var schedule = set3(this, id2), on = schedule.on, listener = schedule.value[key] == null ? remove2 || (remove2 = styleRemove2(name2)) : void 0;
	...
	var NoteArray = class {
		...
		*[Symbol.iterator]() {
		...
		removeOctaves() {
		this._notes = this._notes.map((note2) => Note_default.from(__spreadProps(__spreadValues({}, note2), {
	...
	function preprocessMusicXmlPart(part, drumInstrumentMap) {
		...
		const tempoChanges = []; //当前小节时间，有多少拍
		const beatTypeChanges = []; //当前时间，
		...
		for (const measure of measures) {
			...
			//小节速度+时间
			try {
				const soundElements = measure.querySelectorAll("sound"); //暂理解为音轨或音色
				for (const element of soundElements) {
					const tempoValue = element.getAttribute("tempo"); //当前小节一分钟有多少拍
					if (tempoValue !== null) {
			...
			try {
				divisions = +measure.querySelectorAll("divisions")[0].innerHTML; //每一拍的长度
			} catch {
			}
			try {
				beats = +measure.querySelectorAll("beats")[0].innerHTML; //beats是分子（一小节有xx拍）
				beatType = +measure.querySelectorAll("beat-type")[0].innerHTML;  //beat-type是分母（以xx音符为一拍）	
			...
			const secondsPerBeat = 1 / (tempo / 60); //每一拍的时长
			try {
			...
			if (measure.querySelectorAll("note").length === 0) {
				const measureDuration = beats * (beatType / 4) * secondsPerBeat; //每小节时长
				currentTime += measureDuration;
			}
			let lastNoteDuration = 0;
			// if (i == 4) {
			// 	console.log(measure.children)
			// }
			let j = 0;
			//遍历小节
			for (const child of measure.children) {
				...
				// 	console.log(child)
				// }
				if (child.nodeName === "backup") { //找到左右手分界点
					hand = 1;
					const duration = +child.querySelectorAll("duration")[0].innerHTML; //占多少拍  /divisions
					const durationInSeconds = getDurationInSeconds(duration, divisions, secondsPerBeat); //时长
					currentTime -= durationInSeconds; //减去小节时长，重新计算时间
				} else if (child.nodeName === "forward") { //暂不了解
				...
				} else if (child.nodeName === "direction") { //强弱快慢，情绪相关（个人理解，暂不影响时间）
				...
				} else if (child.nodeName === "note") { //音符
					const note2 = child;
					try { //暂不理解
						let durationInSeconds;
						if (note2.querySelectorAll("grace").length > 0) {
							const type3 = note2.querySelectorAll("type").textContent;
							if (type3 === "64th") {
						...
						if (isRest) {
							...
							const staff = +(((_c = note2.querySelectorAll("staff")[0]) == null ? void 0 : _c.innerHTML) ? ((_c = note2.querySelectorAll("staff")[0]) == null ? void 0 : _c.innerHTML) : 1);
							...
							//zxx休止符用0表示
							noteObjs.push(new Note_default(0, currentTime, 0, 0, 0, 0, hand));
						...
						if (isUnpitched) {
							...
							const alter = +(((_a = note2.querySelectorAll("alter")[0]) == null ? void 0 : _a.innerHTML) ? ((_a = note2.querySelectorAll("alter")[0]) == null ? void 0 : _a.innerHTML) : 0);
						...
						const dynamicsTag = (_b = note2.querySelectorAll("dynamics")[0]) == null ? void 0 : _b.children[0];
						if (dynamicsTag) {
							velocity = dynamicsMap.get(dynamicsTag.nodeName);
						}
						const isChord = note2.querySelectorAll("chord").length > 0; //chord表示此note和前面的一起按下
						if (isChord) {
						...
						if (tieElement && tieElement.getAttribute("type") === "stop") {
							...
							for (let index16 = noteObjs.length - 1; index16 > 0; index16--) {
								...
								if (noteObject.pitch === pitch) {
									...
									const lyrics = getLyricsFromNote(note2);
									if (lyrics.length > 0) {
										const oldLyrics = noteLyricsMap.get(index16) ? noteLyricsMap.get(index16) : "";
						...
						//zxx 连音符开始,处理连音符两步的情况,暂忽略占三步及以上情况
						else if (tieElement && tieElement.getAttribute("type") === "start") {
							xmlNoteIndices.push([xmlNoteIndexMap.get(note2)]);
							const staff = +(((_c = note2.querySelectorAll("staff")[0]) == null ? void 0 : _c.innerHTML) ? ((_c = note2.querySelectorAll("staff")[0]) == null ? void 0 : _c.innerHTML) : 1);
							const startTime = roundToNDecimals(currentTime, ROUNDING_PRECISION);
							const endTime = roundToNDecimals(currentTime + durationInSeconds, ROUNDING_PRECISION);
							...
							if (string !== null && fret !== null) {
								...
								noteObjs.push(new Note_default(pitch, startTime, velocity, staff - 1, endTime, 1, hand));
							}
							const lyrics = getLyricsFromNote(note2);
							if (lyrics.length > 0) {
								noteLyricsMap.set(noteObjs.length - 1, lyrics);
							...
							if (string !== null && fret !== null) {
								...
								noteObjs.push(new Note_default(pitch, startTime, velocity, staff - 1, endTime, 0, hand));
							}
							const lyrics = getLyricsFromNote(note2);
							if (lyrics.length > 0) {
								noteLyricsMap.set(noteObjs.length - 1, lyrics);
	...
	function handleStaveAndTab(track) {
		...
		for (const note2 of notes) {
			const voice = +(note2.querySelectorAll("voice")[0].innerHTML ? note2.querySelectorAll("voice")[0].innerHTML : 1);
	...
	function metronomeTrackFromMusicPiece(musicPiece, tempoFactor = 1) {
		...
		const track = [];
		let currentTime = 0;
		const initialTimeSig = timeSignatures[0].signature ? timeSignatures[0].signature : [4, 4];
		let [beatCount, beatType] = initialTimeSig;
		const timeSigsTodo = timeSignatures.slice(1);
		const initialTempo = tempos[0].bpm ? tempos[0].bpm : 120;
	...
	function preprocessMidiFileData(data, splitFormat0IntoTracks = true, log = false) {
		...
		const totalTime = max(parsedTracks, (d) => (d == null ? void 0 : d.totalTime) ? (d == null ? void 0 : d.totalTime) : 0);
	...
	function parseMidiTrack(track, timeDivision, tempoChanges, beatTypeChanges, keySignatureChanges, log) {
		var _a;
		const notes = [];
		let tempo = ((_a = tempoChanges[0]) == null ? void 0 : _a.tempo) ? ((_a = tempoChanges[0]) == null ? void 0 : _a.tempo) : 120;
		let currentTick = 0;
		let currentTime;
		...
		for (const event of track.event) {
			...
			if (type3 !== EVENT_TYPES.noteOn && type3 !== EVENT_TYPES.noteOff) {
				continue;
			}
			const [pitch, velocity] = event.data;
			const channel = event.channel;
			const key = `${pitch} ${channel}`;
			if (type3 === EVENT_TYPES.noteOff || type3 === EVENT_TYPES.noteOn && velocity === 0) {
			...
			} else if (type3 === EVENT_TYPES.noteOn) {
		...
		if (notes.length > 0) {
			const parsedTrack = {
				...
				instrument,
				instrumentName: instrumentName ? instrumentName : "Unknown instrument"
			};
			return parsedTrack;
		} else {
			return null;
	...
	function simplifyDrumPitches(notes, replacementMap) {
		...
		const newNote = Note_default.from(__spreadProps(__spreadValues({}, note2), { pitch: newPitch }));
		return newNote;
		});
		return simplified;
	}
	...
	function parse2(str) {
		...
		const type3 = t === "M" ? "majorable" : "perfectable";
		const name2 = "" + num + q;
		const dir = num < 0 ? -1 : 1;
		const simple = num === 8 || num === -8 ? num : dir * (step + 1);
		const alt = qToAlt(type3, q);
	...
	function qToAlt(type3, q) {
		return q === "M" && type3 === "majorable" || q === "P" && type3 === "perfectable" ? 0 : q === "m" && type3 === "majorable" ? -1 : /^A+$/.test(q) ? q.length : /^d+$/.test(q) ? -1 * (type3 === "perfectable" ? q.length : q.length + 1) : 0;
	}
	function pitchName(props) {
		...
		const d = dir < 0 ? "-" : "";
		const type3 = TYPES[step] === "M" ? "majorable" : "perfectable";
		const name2 = d + num + altToQ(type3, alt);
		return name2;
	}
	function altToQ(type3, alt) {
		if (alt === 0) {
		return type3 === "majorable" ? "M" : "P";
		} else if (alt === -1 && type3 === "majorable") {
		return "m";
		} else if (alt > 0) {
		return fillStr("A", alt);
		} else {
		return fillStr("d", type3 === "perfectable" ? alt : alt + 1);
		}
	}
	...
	var index2 = {};
	function get4(type3) {
		return index2[type3] || NoChordType;
	}
	...
	function add(intervals, aliases, fullName) {
		...
		const chord2 = __spreadProps(__spreadValues({}, get3(intervals)), {
		name: fullName || "",
		quality,
		intervals,
		aliases
	...
	var index3 = {};
	function get5(type3) {
		return index3[type3] || NoScaleType;
	}
	...
	function add2(intervals, name2, aliases = []) {
		const scale2 = __spreadProps(__spreadValues({}, get3(intervals)), { name: name2, intervals, aliases });
	...
	SCALES.forEach(([ivls, name2, ...aliases]) => add2(ivls.split(" "), name2, aliases));
	...
	function tokenize(name2) {
		const [letter, acc, oct, type3] = tokenizeNote(name2);
		if (letter === "") {
		return ["", name2];
		}
		if (letter === "A" && type3 === "ug") {
		return ["", "aug"];
		}
		if (!type3 && (oct === "4" || oct === "5")) {
		return [letter + acc, oct];
		}
		if (NUM_TYPES.test(oct)) {
		return [letter + acc, oct + type3];
		} else {
		return [letter + acc + oct, type3];
		}
	}
	function get6(src) {
		...
		const [tonic, type3] = tokenize(src);
		const chord2 = getChord(type3, tonic);
		return chord2.empty ? getChord(src) : chord2;
		}
	}
	function getChord(typeName, optionalTonic, optionalRoot) {
		const type3 = get4(typeName);
		const tonic = note(optionalTonic || "");
		const root2 = note(optionalRoot || "");
		if (type3.empty || optionalTonic && tonic.empty || optionalRoot && root2.empty) {
		return NoChord;
		}
		const rootInterval = distance(tonic.pc, root2.pc);
		const rootDegree = type3.intervals.indexOf(rootInterval) + 1;
		if (!root2.empty && !rootDegree) {
		return NoChord;
		}
		const intervals = Array.from(type3.intervals);
		...
		typeName = type3.aliases.indexOf(typeName) !== -1 ? typeName : type3.aliases[0];
		const symbol = `${tonic.empty ? "" : tonic.pc}${typeName}${root2.empty || rootDegree <= 1 ? "" : "/" + root2.pc}`;
		const name2 = `${optionalTonic ? tonic.pc + " " : ""}${type3.name}${rootDegree > 1 && optionalRoot ? " over " + root2.pc : ""}`;
		return __spreadProps(__spreadValues({}, type3), {
		name: name2,
		symbol,
		type: type3.name,
		root: root2.name,
	...
	function transpose2(chordName, interval3) {
		const [tonic, type3] = tokenize(chordName);
		if (!tonic) {
		return chordName;
		}
		return transpose(tonic, interval3) + type3;
	}
	...
	function parse3(src) {
		const [name2, acc, roman, chordType2] = tokenize2(src);
		if (!roman) {
	...
	var mapScaleToType = (scale2, list, sep = "") => list.map((type3, i) => `${scale2[i]}${sep}${type3}`);
	...
	function toMode(mode2) {
		...
		const intervals = get5(name2).intervals;
		return {
		empty: false,
		intervals,
		modeNum,
	...
	function tokenize3(name2) {
		...
		const type3 = name2.substring(tonic.name.length + 1);
		return [tonic.name, type3.length ? type3 : ""];
	}
	function get9(src) {
		...
		const name2 = tonic ? tonic + " " + type3 : type3;
		return __spreadProps(__spreadValues({}, st), { name: name2, type: type3, tonic, notes });
	}
	...
	function convertNotesToTab(notes, tuning, mode2 = "letter", restSize = 0.1) {
		...
		let chordString = chord2.map((note2) => {
			...
			return mode2 === "letter" ? (((_a = getMidiNoteByNr(note2.pitch)) == null ? void 0 : _a.name) ? ((_a = getMidiNoteByNr(note2.pitch)) == null ? void 0 : _a.name) : note2.pitch) : note2.pitch;
			}
		}).join(" ");
		if (chord2.length > 1) {
	...
	function convertNotesToHtmlTab(notes, tuning, mode2 = "letter", restSize = 0.1, colormap = () => "black") {
		...
		let chordString = chord2.map((note2) => {
			...
			string = mode2 === "letter" ? (((_a = getMidiNoteByNr(note2.pitch)) == null ? void 0 : _a.name) ? ((_a = getMidiNoteByNr(note2.pitch)) == null ? void 0 : _a.name) : note2.pitch) : note2.pitch;
			}
	...
	function compress(sequence) {
		...
		const depth = Math.max((pre == null ? void 0 : pre.depth) ? (pre == null ? void 0 : pre.depth) : 0, (repetition == null ? void 0 : repetition.depth) ? (repetition == null ? void 0 : repetition.depth) : 0 + 1, (post == null ? void 0 : post.depth) ? (post == null ? void 0 : post.depth) : 0);
		const length = ((pre == null ? void 0 : pre.length) ? (pre == null ? void 0 : pre.length) : 0) + ((repetition == null ? void 0 : repetition.length) ? (repetition == null ? void 0 : repetition.length) : 0) + ((post == null ? void 0 : post.length) ? (post == null ? void 0 : post.length) : 0);
		return {
		pre,
		seq: repetition,
		rep,
...
//# sourceMappingURL=musicvislib.js.map